...
PFX_INLINE pfx_stream_format_t* init_input_format_ex(pfx_stream_format_t PARAM_INOUT* pformat,
 ...
 PFX_CORE_API pfx_result_t string_a_parse_int	(const pfx_char_t*						PARAM_IN			pstring_a,
														 const	pfx_stream_format_t*		PARAM_IN			pformat,
														 pfx_sint_t*									PARAM_INOUT	parse_result,
														 pfx_index_t*									PARAM_INOUT	pstring_a_offset);

 PFX_CORE_API pfx_result_t string_a_parse_int64 (const pfx_char_t*						PARAM_IN			pstring_a,
															  const	pfx_stream_format_t*		PARAM_IN			pformat,
															  pfx_s64_t*									PARAM_INOUT	parse_result,
															  pfx_index_t*								PARAM_INOUT	pstring_a_offset);

PFX_CORE_API pfx_result_t string_a_parse_float (const pfx_char_t*						PARAM_IN			pstring_a,
															const pfx_stream_format_t*	PARAM_IN			pformat,
															pfx_float_t*								PARAM_INOUT	parse_result,
															pfx_index_t*								PARAM_INOUT	pstring_a_offset);

PFX_CORE_API pfx_result_t	string_a_parse_double (const pfx_char_t*					PARAM_IN			pstring_a,
																 const pfx_stream_format_t*	PARAM_IN			pformat,
																 pfx_double_t*							PARAM_INOUT	parse_result,
																 pfx_index_t*								PARAM_INOUT	pstring_a_offset);


PFX_CORE_API pfx_usize_t sint_parse_string_a (pfx_sint_t									integer,
														pfx_char_t* PARAM_INOUT		pstring_buffer,
														pfx_usize_t									nstring_buffer_max_size,
														const	pfx_stream_format_t*	PARAM_IN			pformat);

PFX_CORE_API pfx_usize_t uint_parse_string_a (pfx_uint_t									integer,
														pfx_char_t* PARAM_INOUT		pstring_buffer,
														pfx_usize_t									nstring_buffer_max_size,
														const	pfx_stream_format_t*	PARAM_IN			pformat);

PFX_CORE_API pfx_usize_t sint64_parse_string_a (pfx_s64_t								long_integer,
															pfx_char_t* PARAM_INOUT	pstring_buffer,
															pfx_usize_t								nstring_buffer_max_size,
															const	pfx_stream_format_t*	PARAM_IN			pformat);

PFX_CORE_API pfx_usize_t uint64_parse_string_a (pfx_u64_t								long_integer,
															pfx_char_t* PARAM_INOUT	pstring_buffer,
															pfx_usize_t								nstring_buffer_max_size,
															const	pfx_stream_format_t*	PARAM_IN			pformat);

PFX_CORE_API pfx_usize_t float_parse_string_a  (pfx_float_t								single_real,
															pfx_char_t* PARAM_INOUT	pstring_buffer,
															pfx_usize_t								nstring_buffer_max_size,
															const	pfx_stream_format_t*	PARAM_IN			pformat);

PFX_CORE_API pfx_usize_t double_parse_string_a  (pfx_double_t							double_real,
																pfx_char_t* PARAM_INOUT	pstring_buffer,
																pfx_usize_t								nstring_buffer_max_size,
																const	pfx_stream_format_t*	PARAM_IN			pformat);


PFX_CORE_API pfx_index_t get_input_stream_format_a (const pfx_char_t*			PARAM_IN		pstream_buffer,
																	pfx_usize_t													nstream_buffer_size,
																	const	pfx_char_t*		PARAM_IN			pformat,
																	pfx_stream_format_t*	PARAM_INOUT	pformat_result);

PFX_CORE_API pfx_index_t get_output_stream_format_a (const pfx_char_t*			PARAM_IN		pstream_buffer,
																		pfx_usize_t													nstream_buffer_size,
																		const	pfx_char_t*		PARAM_IN			pformat,
																		pfx_stream_format_t*	PARAM_INOUT	pformat_result);

PFX_CORE_API pfx_index_t get_input_stream_format_w (const pfx_wchar_t*			PARAM_IN		pstream_buffer,
																		pfx_usize_t													nstream_buffer_size,
																		const	pfx_char_t*		PARAM_IN			pformat,
																		pfx_stream_format_t*	PARAM_INOUT	pformat_result);

PFX_CORE_API pfx_index_t get_output_stream_format_w (const pfx_wchar_t*			PARAM_IN		pstream_buffer,
																			pfx_usize_t													nstream_buffer_size,
																			const	pfx_char_t*		PARAM_IN			pformat,
																			pfx_stream_format_t*	PARAM_INOUT	pformat_result);

PFX_CORE_API pfx_index_t	pfx_steam_sscanf	(const pfx_char_t*		PARAM_IN		pstream_buffer,
											 pfx_usize_t											nstream_buffer_size,
											 const	pfx_char_t*		PARAM_IN		pformat,
											 va_list	args);

PFX_CORE_API pfx_index_t	pfx_stream_ssprintf	(pfx_char_t*					PARAM_INOUT		pstream_buffer,
												 pfx_usize_t													nstream_buffer_size,
												 const	pfx_char_t*		PARAM_IN				pformat,
												 va_list	args);
