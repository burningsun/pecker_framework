...
public:
	static pfx_result_t set_install_apkfile_path(const pfx_char_t* pstr_path,pfx_nsize_t path_length);
	static const const_string_chars& get_install_apkfile_path();
	static HANDLE get_private_apkfile_manager();
	static pfx_boolean_t is_file_exists(const pfx_char_t* pstr_path,pfx_nsize_t path_length);
...
public:
	...
	virtual pfx_result_t open(const pfx_char_t* pstr_path,pfx_nsize_t path_length, pfx_flag_t nOpenType);
	virtual Bytes& read_to_memery(Bytes& memery_buffer,UInt origin = FILE_SEEK_SET,Long offset = 0);
	virtual pfx_nsize_t write_to_file(const const_bytes&  memery_buffer,UInt origin = FILE_SEEK_SET,Long offset = 0);
	virtual pfx_nsize_t get_file_size();
	virtual pfx_result_t close();
