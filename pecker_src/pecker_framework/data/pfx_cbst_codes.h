...
PFX_CBST_TEMPLATE_DEFINES
pfx_cbst PFX_CBST_TEMPLATE_PARAMS ::pfx_cbst(const pfx_cbst PFX_CBST_TEMPLATE_DEFINES* other_ptr) throw (pfx_result_t) : m_root_ptr (null)
{
	if (this != other_ptr)
	{
		...
		if (null == temp_dec_node_ptr->get_left_node())
		{
			if (null != temp_src_node_ptr->get_left_node())
			{
				...
				*new_node_ptr = *(temp_src_node_ptr->get_left_node());
				...
				temp_dec_node_ptr->set_left_node ((node_type_*)new_node_ptr);
				temp_dec_node_ptr = new_node_ptr;

				temp_src_node_ptr = temp_src_node_ptr->get_left_node ();
				continue;
			}
		}
		...
		if (null == temp_dec_node_ptr->get_right_node())
		{
			if (null != temp_src_node_ptr->get_right_node())
			{
				...
				*new_node_ptr = *(temp_src_node_ptr->get_right_node());
				...
				temp_dec_node_ptr->set_right_node ((node_type_*)new_node_ptr);
				temp_dec_node_ptr = new_node_ptr;

				temp_src_node_ptr = temp_src_node_ptr->get_right_node ();
				continue;
			}
		}

		temp_dec_node_ptr = temp_dec_node_ptr->get_parent_node ();
		temp_src_node_ptr = temp_src_node_ptr->get_parent_node();
...
PFX_CBST_TEMPLATE_DEFINES
pfx_result_t pfx_cbst PFX_CBST_TEMPLATE_PARAMS ::copy_by_iterator (const pfx_cbst PFX_CBST_TEMPLATE_PARAMS * PARAM_IN other_bst)
{
	RETURN_RESULT (other_bst_ptr == this,PFX_STATUS_OK);
	pfx_result_t status_ = clear();
	RETURN_INVALID_RESULT (PFX_STATUS_OK != status_,PFX_STATUS_ERROR_);
	...
	pfx_preorder_iterator PFX_CBST_ITERATOR_TEMPLATE_PARAMS*  iterator_begin_ptr = begin (&iterator_begin);
	pfx_preorder_iterator PFX_CBST_ITERATOR_TEMPLATE_PARAMS*  iterator_end_ptr = end (&iterator_end);
	...
	do 
	{
		...
		if (cur_node_ptr && cur_node_ptr == iterator_end_ptr->get_current_node ())
		{
			...
			*new_node_ptr = *(cur_node_ptr);
			...
			add_node_ptr = add (m_root_ptr,new_node_ptr,status_);

			if (null == add_node_ptr)
			{
				remove_node (new_node_ptr);
			}

			BREAK_LOOP (status_,PFX_STATUS_OK);
		}

		iterator_begin_ptr = iterator_begin_ptr->next();
...
pfx_result_t pfx_cbst PFX_CBST_TEMPLATE_PARAMS ::clear ()
{
	...
	pfx_posorder_iterator PFX_CBST_ITERATOR_TEMPLATE_PARAMS*  iterator_begin_ptr = begin (&iterator_begin);
	pfx_posorder_iterator PFX_CBST_ITERATOR_TEMPLATE_PARAMS*  iterator_end_ptr = end (&iterator_end);
...
PFX_CBST_TEMPLATE_DEFINES
pfx_cbst_iterator* pfx_cbst PFX_CBST_TEMPLATE_PARAMS ::begin (pfx_cbst_iterator* PARAM_OUT iterator_) const
{
	if (iterator_)
	{
...
PFX_CBST_TEMPLATE_DEFINES
pfx_cbst_iterator* pfx_cbst PFX_CBST_TEMPLATE_PARAMS ::end (pfx_cbst_iterator* PARAM_OUT iterator_) const
{
	if (iterator_)
	{
		pfx_result_t status_ = iterator_->init (m_root_ptr,m_root_ptr);
...
//public:
PFX_CBST_TEMPLATE_DEFINES
pfx_cbst_iterator* pfx_cbst PFX_CBST_TEMPLATE_PARAMS ::find (pfx_cbst_iterator* PARAM_OUT iterator_,
...
{
	const node_type_* traval_node_ptr = m_root_ptr;
	while (traval_node_ptr)
	{
		int cmp_result = compare_two_node_ (key_node_ptr,traval_node_ptr);
		if (cmp_result < 0)
		{
	...
	pfx_result_t& PARAM_INOUT last_cmp_result)
{
	...
	while (traval_node_ptr)
	{
		cmp_result = compare_two_node_ (key_node_ptr,traval_node_ptr);
		...
		node_type_*& PARAM_OUT added_node_ptr)
{
	...
	parent_node_ptr = node_to_be_add_ptr->get_parent_node() ;
	...
	if (0 > cmp_result)
	{
		if (null == node_to_be_add_ptr->get_left_node())
		{
			...
			node_to_be_add_ptr->set_left_node ((node_type_*)padd_node);
		}
		else
		{
			 // 内存数据被损坏
		...
		const node_type_* PARAM_IN null_node_ptr /*= null*/)
{
	...
	remove_node_ptr = pfx_cbst PFX_CBST_TEMPLATE_PARAMS ::remove_node_internal (root_node_ptr,remove_node_ptr,sub_remove_node_ptr,
	...
	const node_type_* PARAM_IN null_node_ptr /*= null*/)
{
	...
	else
	{
		...
		{
			sub_remove_ref_node_ptr = tmp_sub_remove_node_ptr->get_pright_node ();
		}
	}
...
//
PFX_CBST_TEMPLATE_DEFINES
node_type_* pfx_cbst PFX_CBST_TEMPLATE_PARAMS ::remove_node_internal (node_type_*& PARAM_INOUT root_node_ptr,
		node_type_* PARAM_INOUT remove_node_ptr,
		node_type_* PARAM_INOUT sub_remove_node_ptr,
		node_type_* PARAM_INOUT sub_remove_node_ref_ptr,
		const node_type_* PARAM_IN null_node_ptr /*= null*/)
{
	node_type_* proot_node = root_node_ptr;
	node_type_* parant_node_ptr = null;
	pfx_result_t status_ = PFX_STATUS_OK;

	FOR_ONE_LOOP_BEGIN
	// 处理替换节点
	if (sub_remove_node_ptr == proot_node) 
	{                             
		parant_node_ptr = (node_type_*) (proot_node->get_parent_node ());
		if (null != parant_node_ptr)
		{
			if (parant_node_ptr->get_left_node() == proot_node)
			{
				parant_node_ptr->set_left_node ((node_type_*)sub_remove_node_ref_ptr);
			}
			else if (parant_node_ptr->get_right_node() == proot_node)
			{
				parant_node_ptr->set_right_node ((node_type_*)sub_remove_node_ref_ptr);
			}
			else
			{
		...
		{
			sub_remove_node_ref_ptr->set_parent_node ((node_type_*)parant_node_ptr);
		}
	...
	parant_node_ptr =  (node_type_*) (sub_remove_node_ptr->get_parent_node ());
	if (sub_remove_node_ptr == parant_node_ptr->get_left_node ())
	{
		sub_remove_node_ptr->set_left_node ((node_type_*)sub_remove_node_ref_ptr);
	}
	else
	{
		sub_remove_node_ptr->set_right_node ((node_type_*)sub_romove_ref_node_ptr);
	}
	...
	if (parant_node_ptr == remove_node_ptr)
	{
		if (sub_remove_node_ref_ptr)
		{
			sub_remove_node_ref_ptr->set_parent_node ((node_type_*)sub_remove_node_ptr);
		}
	}
	else
	{
	...
	// 将remove node相关的指针复制
	sub_remove_node_ptr->set_left_node		 ((node_type_*)remove_node_ptr->get_left_node ());
	sub_remove_node_ptr->set_right_node	 ((node_type_*)remove_node_ptr->get_right_node ());
	sub_remove_node_ptr->set_parent_node ((node_type_*)remove_node_ptr->get_parent_node ());


	if (remove_node_ptr == proot_node) // 移除节点是根节点
	{
		root_node_ptr = sub_remove_node_ptr;		// 将根节点替换为psub_node
	}
	else
	{
		parant_node_ptr = (node_type_*)(remove_node_ptr->get_parent_node ());
		// 替换移除节点父节点的相关指针
		if (remove_node_ptr == parant_node_ptr->get_left_node())
		{
			parant_node_ptr->set_left_node ((node_type_*)sub_remove_node_ptr);
		}
		else if (remove_node_ptr == parant_node_ptr->get_right_node())
		{
			parant_node_ptr->set_right_node ((node_type_*)sub_remove_node_ptr);
		}
		else
		{
			BREAK_LOOP (status_,PFX_STATUS_MEM_ERR);
	...
	// 更新现在新的左右节点的父节点指针
	node_type_* left_node_ptr = (node_type_*) (sub_remove_node_ptr->get_left_node ());
	node_type_* right_node_ptr = (node_type_*) (sub_remove_node_ptr->get_right_node ());

	if (null_node_ptr)
	{
		if (left_node_ptr != null_node_ptr && left_node_ptr)
		{
			left_node_ptr->set_parent_node ((node_type_*)sub_remove_node_ptr);
		}

		if (right_node_ptr != null_node_ptr && right_node_ptr)
		{
			right_node_ptr->set_parent_node ((node_type_*)sub_remove_node_ptr);
		}
	}
	else
	{
...
node_type_* pfx_cavl_tree PFX_CBST_TEMPLATE_PARAMS ::remove (node_type_* PARAM_INOUT remove_node_ptr,
		pfx_result_t& PARAM_OUT status_)
{
	return remove_node (m_root_ptr,remove_node_ptr,status_);
}
...
{
	node_type_* rotate_node_ptr = (node_type_*) node_ptr->get_right_node ();

	node_ptr->set_right_node ((node_type_*)rotate_node_ptr->get_left_node ());
	rotate_node_ptr->set_left_node ((node_type_*)node_ptr);

	node_type_* tmp_right_node_ptr = (node_type_*) (node_ptr->get_right_node ());
	if (tmp_right_node_ptr)
	{
		tmp_right_node_ptr->set_parent_node (node_ptr);
	}
	rotate_node_ptr->set_parent_node ((node_type_*)node_ptr->get_parent_node ());
...
{
	node_type_* rotate_node_ptr = (node_type_*)node_ptr->get_left_node (); 

	node_ptr->set_left_node ((node_type_*)rotate_node_ptr->get_right_node ());
	rotate_node_ptr->set_right_node ((node_type_*)node_ptr);

	node_type_* tmp_left_node_ptr = (node_type_*) (node_ptr->get_left_node ());
	...
	update_avl_node_height_unsafe (node_ptr);
	update_avl_node_height_unsafe (rotate_node_ptr);
...
{
	node_type_* tmp_node_ptr = avl_rr_rotate ((node_type_*)node_ptr->get_left_node ());
...
{
	node_type_* tmp_node_ptr = avl_ll_rotate ((node_type_*)node_ptr->get_right_node ());
	...
	node_type_* PARAM_INOUT node_ptr, node_type_* & PARAM_INOUT parent_ref_node_ptr)
{
	...
	if (2 == balance_value)
	{
		node_type_ * left_node_ptr = (node_type_ *) node_ptr->get_left_node ();
	...
	if(-2 == balance_value)
	{
		node_type_ * right_node_ptr =(node_type_*)(node_ptr->get_right_node ());
	...
	node_type_* PARAM_INOUT begin_node_ptr)
{
	pfx_result_t			status = PFX_STATUS_OK;
	pfx_nsize_t				height;
	node_type_*			proot_node;
	node_type_*			parent_node;
	AVLTREE_ROTATE_t rotate_type;

	proot_node = *root_node_ptr;


	if (proot_node == begin_node_ptr)
	{
		rotate_type = pfx_cavl_tree PFX_CBST_TEMPLATE_PARAMS ::avl_single_rotate 
			(pfx_cavl_tree PFX_CBST_TEMPLATE_PARAMS ::calculate_avl_balance_value (proot_node), proot_node, root_node_ptr);
	...
	do 
	{
		...
		node_type_*& temp_node_ptr_ref;
		parent_node = (node_type_*)(begin_node_ptr->get_parent_node ());
		...
		node_type_* parent_parent_node_ptr = (node_type_*) parent_node->get_parent_node ();

		if (parent_node == proot_node || null == parent_parent_node_ptr)
		{
			temp_node_ptr_ref = root_node_ptr;
		}
		else
		{
			if (parent_parent_node_ptr->get_left_node () == parent_node)
			{
				temp_node_ptr_ref = (node_type_*&) (parent_parent_node_ptr->get_left_node_ref ());
			}
			else if (parent_parent_node_ptr->get_right_node () == parent_node)
			{
				temp_node_ptr_ref = (node_type_*&) (parent_parent_node_ptr->get_right_node_ref ());
			}
			else
			{
				BREAK_LOOP (status,PFX_STATUS_MEM_ERR);
...
{
	pfx_result_t			status;
	avl_tree_node_t*	temp_node_ptr = null;
	...
	pfx_result_t& PARAM_OUT status_)
{
	...
	if (null == temp_node_ptr)
	{
		...
		{
			parent_node_ptr = (node_type_*)sub_remove_node_ptr->get_parent_node();
	...
	node_type_ *	PARAM_INOUT node_ptr)
{
	...
	temp_node_ptr = (node_type_ *) node_ptr->get_right_node ();
	node_ptr->set_right_node ((node_type_ *) temp_node_ptr->get_left_node());

	tmp_letf_node_ptr = (node_type_ *) temp_node_ptr->get_left_node ();
	if (tmp_letf_node_ptr != sentinel_node_ptr) 
	{
		tmp_letf_node_ptr->set_parent_node (node_ptr);
	}

	temp_node_ptr->set_parent_node ((node_type_ *)node_ptr->get_parent_node());
	...
	{
		node_type_ * tmp_parent_node_ptr = (node_type_ *) node_ptr->get_parent_node ();
	...
	node_type_ *	PARAM_INOUT	node_ptr)
{
	...
	temp_node_ptr = (node_type_ *) node_ptr->get_left_node ();
	node_ptr->set_left_node ((node_type_ *) temp_node_ptr->get_right_node ());

	temp_right_node_ptr = (node_type_ *) temp_node_ptr->get_right_node ();
	...
	{
		node_type_ * tmp_parent_node_ptr = node_ptr->get_parent_node ();
	...
	node_type_*  PARAM_INOUT add_node_ptr)
{
	...
	{
		is_color_temp = pfx_crb_tree PFX_CBST_TEMPLATE_PARAMS :: is_rbt_color_red 
			((node_type_*)add_node_ptr->get_parent_node ());
		...
		node_type_* parent_node_ptr = (node_type_*) add_node_ptr->get_parent_node ();
		node_type_* parent_parent_node_ptr =  (node_type_*) parent_node_ptr->get_parent_node ();

		RETURN_INVALID_RESULT (null == parent_parent_node_ptr,PFX_STATUS_MEM_ERR);

		if (parent_parent_node_ptr->get_left_node () == parent_node_ptr) 
		{
			temp_node_ptr = parent_parent_node_ptr->get_right_node (); 
			is_color_temp = pfx_crb_tree PFX_CBST_TEMPLATE_PARAMS :: is_rbt_color_red (temp_node_ptr);
			...
			if (is_color_temp)
			{
				...
				pfx_crb_tree PFX_CBST_TEMPLATE_PARAMS :: set_rbt_color_red (parent_parent_node_ptr);
				add_node_ptr = parent_parent_node_ptr;
			}
			else
			{
				if (parent_node_ptr->get_right_node () == add_node_ptr)
				{
					...
					parent_parent_node_ptr =  (node_type_*) parent_node_ptr->get_parent_node ();
				}
				pfx_crb_tree PFX_CBST_TEMPLATE_PARAMS :: set_rbt_color_black (parent_node_ptr);
				pfx_crb_tree PFX_CBST_TEMPLATE_PARAMS :: set_rbt_color_red (parent_parent_node_ptr);
				pfx_crb_tree PFX_CBST_TEMPLATE_PARAMS :: rbt_right_rotate (root_node_ptr,null,parent_parent_node_ptr);
			}

		} 
		else 
		{
			temp_node_ptr = parent_parent_node_ptr->get_left_node ();
			is_color_temp =  pfx_crb_tree PFX_CBST_TEMPLATE_PARAMS :: is_rbt_color_red (temp_node_ptr);
			...
			else
			{
				...
				pfx_crb_tree PFX_CBST_TEMPLATE_PARAMS :: set_rbt_color_black (parent_node_ptr);
				pfx_crb_tree PFX_CBST_TEMPLATE_PARAMS :: set_rbt_color_red (parent_parent_node_ptr);
				pfx_crb_tree PFX_CBST_TEMPLATE_PARAMS :: rbt_left_rotate (root_node_ptr,null,parent_parent_node_ptr);
			}
		}
	}
...
{
	node_type_* temp_node_ptr;
	pfx_bool_t is_color_temp = false;
	pfx_bool_t is_color_temp_ = false;

	RETURN_INVALID_RESULT ((null == ref_node_ptr),PFX_STATUS_OK);

	while (ref_node_ptr != root_node_ptr)
	{
		is_color_temp = pfx_crb_tree PFX_CBST_TEMPLATE_PARAMS :: is_rbt_color_black (ref_node_ptr);
		...
		node_type_* parent_node_ptr = (node_type_*) ref_node_ptr->get_parent_node ();

		if (parent_node_ptr->get_left_node() == ref_node_ptr)
		{
			temp_node_ptr = parent_node_ptr->get_right_node ();
			is_color_temp = pfx_crb_tree PFX_CBST_TEMPLATE_PARAMS :: is_rbt_color_red (temp_node_ptr);
			...
			is_color_temp = pfx_crb_tree PFX_CBST_TEMPLATE_PARAMS :: is_rbt_color_black 
				((node_type_*) temp_node_ptr->get_left_node ()) ;

			is_color_temp_ = pfx_crb_tree PFX_CBST_TEMPLATE_PARAMS :: is_rbt_color_black 
				((node_type_*) temp_node_ptr->get_right_node ());

			if (is_color_temp && is_color_temp_)
			{
				pfx_crb_tree PFX_CBST_TEMPLATE_PARAMS :: set_rbt_color_red (temp_node_ptr);
				pref_node = parent_node_ptr;
				//parent_node_ptr = (node_type_*) ref_node_ptr->get_parent_node ();
			}
			else
			{
				is_color_temp = pfx_crb_tree PFX_CBST_TEMPLATE_PARAMS :: is_rbt_color_black 
				...
				if (is_color_temp)
				{
					node_type_* temp_left_node_ptr = temp_node_ptr->get_left_node ();
				...
				node_type_* temp_right_node_ptr = temp_node_ptr->get_right_node ();
		...
		else
		{
			...
			else
			{
				...
				node_type_* temp_left_node_ptr = temp_node_ptr->get_right_node ();
...
{
	pfx_result_t			status;
	redblack_tree_node_t*	temp_node_ptr = null;
	...
	// 平衡变换
	status = pfx_cbst PFX_CBST_TEMPLATE_PARAMS ::rbt_add_rotate_fixup (root_node_ptr,add_node_ptr);
	...
	pfx_result_t& PARAM_OUT status_)
{
	...
	is_red = pfx_crb_tree PFX_CBST_TEMPLATE_PARAMS :: is_rbt_color_red (sub_remove_node_ptr);
	...
	RETURN_RESULT ((is_red || null == root_node_ptr),remove_node_ptr);
