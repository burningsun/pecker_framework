...
{
	typedef vector_dim_op_alg < dim_value, optional_type > dim_alg_t;
...
struct PFX_DATA_TEMPLATE_API vector_base_op_alg
{
	...
	static PFX_INLINE const vector_t* matrix_at_const (const matrix_t& __matrix, uindex_t i);
	static PFX_INLINE const vector_t* const& matrix_at_const (const matrix_ex_t& __matrix, uindex_t i);

	static PFX_INLINE vector_t& init_vector (vector_t& __vector, const dim_t __val);
	static PFX_INLINE vector_t& init_vector (vector_t& __vector, const dim_t __val, usize__t dim_count);
	static PFX_INLINE vector_t& init_vector (vector_t& __vector, const vector_t& other_vec);
	static PFX_INLINE vector_t& init_vector (vector_t& __vector, const vector_t& other_vec, usize__t dim_count);

	static PFX_INLINE matrix_ex_t& init_matrix (matrix_ex_t& __matrix, const matrix_t& s_matrix, usize__t vector_count);
	static PFX_INLINE matrix_ex_t& init_matrix (matrix_ex_t& __matrix, const dim_t __val , usize__t vector_count);
	static PFX_INLINE matrix_t& init_matrix (matrix_t& __matrix, const dim_t __val , usize__t vector_count);
	...
	static PFX_INLINE dim_t& vector_dot (usize__t dim_count,
	...
	static PFX_INLINE vector_t& vector_mul (const vector_t& __vec_a, const dim_t& __dim,
			usize__t dim_count, vector_t& PARAM_INOUT __vec_result);
...
struct PFX_DATA_TEMPLATE_API vector_base_op_traits
{
	...
	typedef vector_base_op_alg < vector_type, optional_type > vec_alg_t;
	...
	typedef typename vector_dim_op_traits < dim_t, optional_type >::dim_alg_t dim_alg_t;
};

template < class square_matrix_type, const enum_int_t optional_type = 0 >
struct square_matrix_base_op_alg
{
	typedef square_matrix_type smatrix_t;
	...
	{
		friend struct square_matrix_base_op_alg < smatrix_t, optional_type >;
	protected:
		dim_t m_factor;
		uindex_t m_index;
	}clr_mul_factor_t;
	...
	static PFX_INLINE cw_buffer_t new_cw_buffer (usize__t square_dim);
	static PFX_INLINE result_t release_cw_buffer ();
protected:
	static PFX_INLINE typename cw_buffer_t::__vector_t* get_cw_matrix (cw_buffer_t& __cw_buf,
			uindex_t step_index, uindex_t mat_index);
public:

	static PFX_INLINE usize__t get_dim_count ();

	static PFX_INLINE matrix_t& Indentify_matrix (usize__t dim_count,
			matrix_t& __mat, dim_t val, dim_t null_val);

	static PFX_INLINE matrix_ex_t& Indentify_matrix (usize__t dim_count,
			matrix_ex_t& __mat, dim_t val, dim_t null_val);

	static PFX_INLINE matrix_t& transpose_matrix (usize__t dim_count,
			const matrix_t& __mat,
			matrix_t& __mat_reslut);

	static PFX_INLINE matrix_ex_t& transpose_matrix (usize__t dim_count,
			const matrix_ex_t& __mat,
			matrix_ex_t& __mat_reslut);
	...
	static PFX_INLINE vector_t& vector4_mul(const vector_t& __mat_a, const matrix_ex_t& __mat_b,
			vector_t& PARAM_INOUT __mat_result,
			vector_t* PARAM_INOUT __tempbuffer = NULL,
			u32_t delta = MID_PRECISION_QUALIFER_VAL);
	...
	static PFX_INLINE matrix_ex_t& matrixNxN_mul (const matrix_ex_t& __mat_a, const matrix_ex_t& __mat_b,
			...
			u32_t delta = MID_PRECISION_QUALIFER_VAL);
protected:
	static PFX_INLINE matrix_ex_t& matrixNxN_mul (usize__t dim_count,
...
public:
	static PFX_INLINE void swap_line (matrix_ex_t& PARAM_INOUT __matrix,
			uindex_t i, uindex_t j);

	static PFX_INLINE dim_t clear_oneline (matrix_ex_t& PARAM_INOUT __matrix,
			uindex_t test_line, uindex_t clear_line,
			uindex_t line_index, usize__t dim_count);

	static PFX_INLINE void clear_oneline_same (matrix_ex_t& PARAM_INOUT __matrix,
			uindex_t test_line,
			uindex_t clear_line, dim_t mul_factor,
			usize__t dim_count);

	static PFX_INLINE void restore_oneline (matrix_ex_t& PARAM_INOUT __matrix,
			uindex_t restore_line,uindex_t line_index,
			dim_t mul_factor, usize__t dim_count);

	static PFX_INLINE usize__t rev_clear_oneline (matrix_ex_t& PARAM_INOUT __matrix,
			uindex_t clear_line,
			usize__t dim_count,
			clr_mul_factor_t* mul_factor, u32_t delta);

	static PFX_INLINE void rev_clear_oneline_same (matrix_ex_t& PARAM_INOUT __matrix,
			uindex_t clear_line,
			usize__t dim_count,
	...
	static PFX_INLINE uindex_t find_test_line (matrix_ex_t& PARAM_INOUT __matrix,
			uindex_t line_index, usize__t dim_count);
...
public:
	...
	static PFX_INLINE dim_t matrix1x1_det (const matrix_t& PARAM_INOUT __mat);

	static PFX_INLINE dim_t matrix2x2_det (const matrix_t& PARAM_INOUT __mat);

	static PFX_INLINE dim_t matrix3x3_det (const matrix_t& PARAM_INOUT __mat);
	...
	static PFX_INLINE dim_t matrix4x4_algebraic_cofactor_det (const matrix_ex_t& PARAM_INOUT __mat,
			usize__t x, usize__t y);
	static PFX_INLINE dim_t matrix1x1_algebraic_cofactor_det (const matrix_t& PARAM_INOUT __mat);

	static PFX_INLINE dim_t matrix2x2_algebraic_cofactor_det (const matrix_t& PARAM_INOUT __mat,
			usize__t x, usize__t y);
	static PFX_INLINE dim_t matrix3x3_algebraic_cofactor_det (const matrix_t& PARAM_INOUT __mat,
			usize__t x, usize__t y);
	static PFX_INLINE dim_t matrix4x4_algebraic_cofactor_det (const matrix_t& PARAM_INOUT __mat,
			usize__t x, usize__t y);
	...
	//
	// 求秩 （方阵）
	static PFX_INLINE usize__t matrix_rank (matrix_ex_t& PARAM_INOUT __mat_replace,
			usize__t dim_count, u32_t delta = MID_PRECISION_QUALIFER_VAL);
...
{
	typedef square_matrix_base_op_alg < square_matrix_type, optional_type > matrix_alg_t;
	typedef typename matrix_alg_t::dim_alg_t dim_alg_t;
	typedef typename matrix_alg_t::vec_alg_t vec_alg_t;
	typedef typename matrix_alg_t::dim_t dim_t;
