...
protected:
	typedef struct parasite_node
	{
		...
		// 不过虚表在前还是在后，各个编译器不一样，坑死大爹，gcc在前面，vc的在后面
#ifndef NO_BEGIN_VTABLE
		char_t      m_vtable_b[(_VTABLE_SIZE_ ? _VTABLE_SIZE_ : sizeof(lpointer_t))];
#endif
		parasite_t  m_base;

#ifndef NO_END_VTABLE
		char_t      m_vtable_e[(_VTABLE_SIZE_ ? _VTABLE_SIZE_ : sizeof(lpointer_t))];
#endif
...
protected:
	static PFX_INLINE clist_node_t* born()
	{
		...
		REF_LOG_INFO ("new_ptr = 0x%08X(%d)",new_ptr,sizeof(clist_node_t));
		if (new_ptr)
		{
