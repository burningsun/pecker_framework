...
public:
	...
	virtual  const IString_t*	sub_string (pfx_uindex_t index_,
																																IString_t* PARAM_INOUT sub_string_ptr)  = 0;
	virtual  const IString_t*	sub_string (pfx_uindex_t index_,
																																pfx_usize_t		sub_string_size, 
																																IString_t* PARAM_INOUT sub_string_ptr)  = 0;
	virtual  const IString_t *sub_string_const (pfx_uindex_t index_,
	...
	virtual	 pfx_boolean_t			find_first_string (const IString_t * PARAM_IN sub_string_ptr,pfx_uindex_t & find_index) const = 0;
	...
	virtual pfx_result_t				append_string (const IString_t* PARAM_IN append_string_ptr) = 0;

	virtual pfx_result_t				append_front (const element_* PARAM_IN str_chars_buffer_ptr,pfx_usize_t buffer_size) = 0;
	virtual pfx_result_t				append_front (const IString_t* PARAM_IN append_string_ptr) = 0;

	virtual pfx_result_t				clip_string_remain_left (pfx_uindex_t clip_index, IString_t* PARAM_IN other_ptr) = 0;
	virtual pfx_result_t				clip_string_remain_right (pfx_uindex_t clip_index, IString_t* PARAM_IN other_ptr) = 0;
...
public:
	...
	typedef typename pfx_cstring PFX_CSTRING_TEMPLATE_PARAMS	cstring_t;
	//typedef typename IPfx_string PFX_STRING_TEMPLATE_PARAMS		IString_t;
protected:
...
protected:
	...
	PFX_INLINE pfx_result_t			add_referance (IString_t* PARAM_IN other_ptr);
...
public:
	pfx_cstring ();
	pfx_cstring (const cstring_t & other_) throw (pfx_result_t);
	virtual ~pfx_cstring ();
public:
...
public:
	...
	const IString_t*	sub_string (pfx_uindex_t index_, 
																													IString_t* PARAM_INOUT sub_string_ptr) ;
	const IString_t*	sub_string (pfx_uindex_t index_,
																													pfx_usize_t		sub_string_size, 
																													IString_t* PARAM_INOUT sub_string_ptr) ;

	const IString_t*	sub_string_const (pfx_uindex_t index_,IString_t* PARAM_INOUT sub_string_ptr)  const;

	const IString_t*	sub_string_const (pfx_uindex_t index_,
																																pfx_usize_t		sub_string_size, 
	 ...
	 pfx_boolean_t			find_first_string (const IString_t * PARAM_IN find_string_ptr,pfx_uindex_t & find_index) const;
	 ...
	 pfx_boolean_t			find_string (pfx_uindex_t begin_index, IString_t * PARAM_INOUT sub_string_ptr,pfx_uindex_t& find_index) const;
	 ...
	 pfx_boolean_t			find_near_string (pfx_uindex_t begin_index, IString_t * PARAM_INOUT sub_string_ptr,pfx_uindex_t& find_index, pfx_usize_t& same_chars_count) const;
	...
	pfx_result_t				init_string (const IString_t * PARAM_IN other_ptr);
	...
	pfx_result_t				append_string (const IString_t* PARAM_IN append_string_ptr);

	pfx_result_t				append_front (const element_* PARAM_IN str_chars_buffer_ptr,pfx_usize_t buffer_size);
	pfx_result_t				append_front (const IString_t* PARAM_IN append_string_ptr);

	pfx_result_t				clip_string_remain_left (pfx_uindex_t clip_index, IString_t* PARAM_IN other_ptr);
	pfx_result_t				clip_string_remain_right (pfx_uindex_t clip_index, IString_t* PARAM_IN other_ptr);
...
public:
	typedef element_				item_type_t;
	typedef elem_compare		compare_two_item_t;
	typedef pfx_cshare_string PFX_CSTRING_TEMPLATE_PARAMS cshare_string_t;
protected:
...
public:
	pfx_cshare_string ();
	pfx_cshare_string (const cshare_string_t & other_) throw (pfx_result_t);
