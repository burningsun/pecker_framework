	...
	pfx_result_t* PARAM_INOUT pstatus)
{
	...
	else
	{
		...
		preturn_string->m_char_buffer_size = 0;
		preturn_string->m_reference_string = init_string_by_buffer(char_size,min_buffer_size-sizeof(pfx_share_string_t),
	...
	pfx_result_t* PARAM_INOUT pstatus)
{
	...
	preturn_string = new_share_string(porign_string->m_reference_string->m_char_size,0,pchar_allocator,&status);
	...
	const IAllocator* PARAM_IN pchar_allocator)
{
	...
	if (null != pstr->m_reference_string)
	{
		RETURN_INVALID_RESULT ((char_size != pstr->m_reference_string->m_char_size),
			PFX_STATUS_INVALID_PARAMS);
	...
	if (null == pstr->m_reference_string)
	{
		if (string_len > 0)
		{
			pstr->m_reference_string = new_string(char_size,string_default_len,pchar_allocator,&status);
