...
int array_test_main()
{
	...
	pfx_base_array_t*  parray = new_base_array(&gDefualt_allocator,
		sizeof(U64bit),16,16,&gDefualt_allocator,&status);
	...
	{
		U64bit add_item = i;
		U64bit* padd_item = (U64bit*)base_array_push_back(parray,&add_item,&status);
	...
	for (i=0;i<1000;++i)
	{
		U64bit* pitem = (U64bit*)get_item_at_base_array(parray,i);
		if (null == pitem)
		{
	...
	for (i=0;i<1000;++i)
	{
		U64bit* pop_item = (U64bit*)base_array_pop_back(parray,pfx_false,&status);
	...
	for (i=0;i<20;++i)
	{
		U64bit* padd_item = (U64bit*)base_array_push_back_n(parray,push_back_n_items,(sizeof(push_back_n_items)/sizeof(U64bit)),&status);
		PECKER_LOG_("base_array_push_back status = %d\n",status);
		if (null == padd_item)
		{
	...
	{
		size_t ncount = 0;
		U64bit* pop_item = (U64bit*)base_array_pop_back_n(parray,(sizeof(push_back_n_items)/sizeof(U64bit)),pfx_false,&ncount,&status);
