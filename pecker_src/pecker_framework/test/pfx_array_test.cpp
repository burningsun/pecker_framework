...
int array_test_main()
{
	...
	pfx_base_array_t*  parray = new_base_array(&gDefualt_allocator,
		sizeof(pfx_u64_t),16,16,&gDefualt_allocator,&status);
	...
	for (i=0;i<1000;++i)
	{
		...
		pfx_u64_t* padd_item = (pfx_u64_t*)base_array_push_back(parray,&add_item,&status);
	...
	for (i=0;i<1000;++i)
	{
		pfx_u64_t* pitem = (pfx_u64_t*)get_item_at_base_array(parray,i);
		if (null == pitem)
		{
	...
	for (i=0;i<1000;++i)
	{
		pfx_u64_t* pop_item = (pfx_u64_t*)base_array_pop_back(parray,pfx_false,&status);
	...
	status = clear_base_array(parray,pfx_true,INVALID_VALUE);
	...
	for (i=0;i<20;++i)
	{
		pfx_u64_t* padd_item = (pfx_u64_t*)base_array_push_back_n(parray,push_back_n_items,(sizeof(push_back_n_items)/sizeof(pfx_u64_t)),&status);
		PECKER_LOG_("base_array_push_back status = %d\n",status);
		if (null == padd_item)
		{
	...
	{
		size_t ncount = 0;
		pfx_u64_t* pop_item = (pfx_u64_t*)base_array_pop_back_n(parray,(sizeof(push_back_n_items)/sizeof(pfx_u64_t)),pfx_false,&ncount,&status);
	...
	delete_base_array(parray,&gDefualt_allocator);
	parray = null;
