...
typedef pfx_binary_search_tree_type< bst_string_node_t, compare_two_node< bst_string_node_t, bst_string_node_t, string_compare< cstring< allc_char_t > > > >::avl_binary_search_tree_t bst_t;
...
PFX_INLINE_CODE void print_avl_bst_node(const bst_string_node_t* PARAM_IN node_ptr)
{
	if (node_ptr)
	{
		...
		if (str_ptr)
		{
			sprintf_s (strformat, sizeof(strformat), "THIS(%08X) <%%.%ds> L(%08X) R(%08X) P(%08X) HEIGHT(%d) BALANCE(%d)", (ulong_t) node_ptr, string_ref.get_length (),
					(ulong_t) node_ptr->get_left_node (), (ulong_t) node_ptr->get_right_node (), (ulong_t) node_ptr->get_parent_node (), (ulong_t) node_ptr->get_height (),
...
result_t avl_bst_cmd_operate_func(CMD_INOUT_t* cmd_type,
		const char_t* str_chars_ptr,
		usize__t nchars_count)
{
	switch (*cmd_type)
	{
	case CMD_ADD:
