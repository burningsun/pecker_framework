	...
	boolean_t bshow = true)
{
	...
	pecker_tick tick_count;
	__bst_type::const_iterator_t	__itr;
	__bst_type::const_iterator_t* itr_ptr = null;
	...
	while (itr_ptr)
	{
		const __bst_type::node_t* node_ptr;
	...
	__array_type* PARAM_INOUT array_ptr)
{
	...
	while (1)
	{
		...
		__array_type::element_t& elem = array_ptr->get_element_at(old_array_size); 
