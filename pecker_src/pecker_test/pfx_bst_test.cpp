...
typedef cbst_node < cstring < allc_char_t > > bst_string_node_t;
typedef  pfx_binary_search_tree_type < bst_string_node_t, 
	compare_two_node< bst_string_node_t, bst_string_node_t, string_compare < cstring < allc_char_t > > > >::binary_search_tree_t bst_t;
...
PFX_INLINE_CODE void print_bst_node (const bst_string_node_t* PARAM_IN node_ptr)
{
	if (node_ptr)
	{
		...
		if (str_ptr)
		{
			sprintf_s(strformat, sizeof(strformat), "THIS(%08X) <%%.%ds> L(%08X) R(%08X) P(%08X)",
		...
		{
			sprintf_s(strformat, sizeof(strformat), "THIS(%08X) <null> L(%08X) R(%08X) P(%08X)",
				(ulong_t)node_ptr, //string_ref.get_length(),
				(ulong_t)node_ptr->get_left_node(),
				(ulong_t)node_ptr->get_right_node(),
				(ulong_t)node_ptr->get_parent_node());
	...
	case CMD_DEL:
		{
			...
			if (remove_node_ptr)
			{
				...
				{
					print_bst_node(remove_node_ptr);
	...
	case CMD_SEARCH:
		{
			...
			print_bst_node(search_node_ptr);
		}
		break;
	case CMD_COPY:
		{
		...
		PECKER_LOG_STR("============\n");
			if (0 == strncmp(str_chars_ptr,"inorder",strlen("inorder")))
			{
				...
				while (iterator_ptr)
				{
					...
					print_bst_node(iterator_ptr->cur_node());
